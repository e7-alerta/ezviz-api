...
class EzvizClient:
    ...
    def get_cameras(self) -> dict[str, EzvizCamera]:
        """Get cameras."""
        return self._cameras

    def get_camera(self, serial: str) -> EzvizCamera:
        """Get camera by serial."""
        return self._cameras.get(serial)
    ...
    def get_pic(
        ...
        if json_output["resultCode"] != "0":
            if json_output["resultCode"] == "-1":
                ...
                return self.reboot_camera(serial, max_retries + 1)
            raise PyEzvizError(f"Could not collect pic device {json_output})")
